...
class TradingBot:
    def __init__(self):
        ...
        self.stop_loss_ratio = 0.98  # Stop loss
        self.take_profit_ratio = 1.05  #Take profit
    ...
    def long_signal(self, row):
        ...
        williams_r_condition = row['williams_r'] < -234  # Decreased by 50%
    ...
    def short_signal(self, row):
        macd_condition = row['macd'] < row['macdsignal'] - 0.13  # Increased by 50%
        stochrsi_condition = row['stochrsi'] > 241.5  # Increased by 50%
        macdhist_condition = row['macdhist'] < -0.026  # Decreased by 50%
        cci_condition = row['cci'] > 520  # Increased by 50%
        vwap_condition = row['close'] > row['vwap'] + 0.026  # Increased by 50%
        mfi_condition = row['mfi'] > 234  # Increased by 50%
        williams_r_condition = row['williams_r'] > -3.5  # Increased by 50%
        adx_condition = row['adx'] < 17.5  # Unchanged as it's a comparison
        psar_condition = row['close'] < row['psar']  # Unchanged as it's a comparison
    ...
    def close_short_signal(self, row):
        percentage_change = (row['close'] - self.price_last_order) / self.price_last_order
        stop_loss_condition = self.short_balance * self.price_last_order * ( 1 - percentage_change) <= self.total_money_spent * self.stop_loss_ratio
        take_profit_condition = self.short_balance * self.price_last_order * ( 1 - percentage_change) >= self.total_money_spent * self.take_profit_ratio
